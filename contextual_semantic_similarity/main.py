...
language = 'en'
texts_filepath = '../data/meco/supp_texts.csv'  # '../data/Provo/Provo_Corpus-Predictability_Norms.csv' # '../data/meco/supp texts.csv'
eye_move_filepath = '../data/meco/joint_data_trimmed.rda'  # '../data/Provo/Provo_Corpus-Eyetracking_Data.csv' # '../data/meco/joint_data_trimmed.rda'
frequency_filepath = '../data/meco/wordlist_meco.csv'  # '../data/Provo/SUBTLEX_UK.txt' # '../data/meco/wordlist_meco.csv'
...
if compute:
    ...
    words_filepath = f'../data/{corpus_name}/words_{language}_df.csv'
    processed_eye_move_filepath = f'../data/{corpus_name}/corpus_{language}_df.csv'
    if os.path.exists(words_filepath) and os.path.exists(processed_eye_move_filepath):
        ...
        words_df, eye_move_df = pre_process_data(eye_move_filepath, frequency_filepath, texts_filepath, corpus_name, language)
    ...
    surprisal_filepath = f'../data/{corpus_name}/{model_name_dir}/surprisal_{model_name_dir}_{corpus_name}_df.csv'
    if os.path.exists(surprisal_filepath):
        surprisal_df = pd.read_csv(surprisal_filepath, sep='\t')
    else:
        surprisal_df = calculate_surprisal_values(words_df, corpus_name, model_name, model, tokenizer, device)
    ...
    for layer_combi in layers:
        ...
        similarity_filepath = f'../data/{corpus_name}/{model_name_dir}/similarity_{context_type}_{layer_combi}_{model_name}_{corpus_name}_df.csv'
        eye_move_sim_filepath = f'../data/{corpus_name}/{model_name_dir}/full_{model_name_dir}_{layer_combi}_{corpus_name}_{context_type}_df.csv'
...
if evaluate and context_type == 'previous_context':
    evaluation(layers, measures, model_name_dir, corpus_name, context_type, n, seed)